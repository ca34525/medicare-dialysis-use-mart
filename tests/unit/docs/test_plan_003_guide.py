"""Static accessibility and offline-link checks for the Plan 003 HTML guide."""
...
GUIDE_PATH = REPOSITORY_ROOT / "docs" / "guides" / "003-cms-staging-explained.html"
...
class GuideParser(HTMLParser):
    """Collect the small set of structural facts required by the guide policy."""
...
def parsed_guide() -> tuple[str, GuideParser]:
    """Parse the tracked guide and assert that HTML parsing reaches EOF."""
...
def relative_luminance(hex_color: str) -> float:
    """Return WCAG relative luminance for one three- or six-digit hex color."""
...
def contrast_ratio(first: str, second: str) -> float:
    """Return WCAG contrast ratio for two opaque hex colors."""
...
def test_guide_is_network_free_and_contains_no_script() -> None:
    document, parser = parsed_guide()
